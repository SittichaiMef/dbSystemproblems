...
if (isset($_POST["empname"])) {
    if (isset($_POST["year"]) && isset($_POST["month"])) {
        $sql = "SELECT * FROM dowtimedata WHERE problem like '$_POST[empname]%' AND datee LIKE '$_POST[year]-$_POST[month]%'";
    } else {
        $sql = "SELECT * FROM dowtimedata WHERE problem like '$_POST[empname]%';";
    }
    // $key = $_POST["empname"];
} else {
    if (isset($_POST["year"]) && isset($_POST["month"])) {
        $sql = "SELECT * FROM dowtimedata WHERE datee LIKE '$_POST[year]-$_POST[month]%'";
    } else {
        $sql = "SELECT * FROM dowtimedata ;";
    }
}
...
<body>
    <div class="header_img">
        <img src="assets\bg.png" alt="" style="width:100%; height: 280px;">
    </div>

    <div class="container-fluid">

        <ul class="nav nav-tabs nav-tabs_custom">
            <li class="active"><a data-toggle="tab" href="index.php"><i class="fa-solid fa-floppy-disk"></i> ปัญหา Dowtime ระบบ</a></li>
            <li><a href="prindex.php"><i class="fa-solid fa-circle-exclamation"></i> ปัญหาที่เกิดจากโปรแกรม</a></li>
        ...
        <div class="d-flex justify-content-between align-items-center bg-light header_config ">
            ...
            <form id="app-cover" class="d-flex col-12" action="index.php" method="post">
                <div class="row">
                    <div class="input-group col-md-2 col-lg-3 col-sm-12 " >
                        <a href="insertForm.php" class="btn btn-success" style="width: 75%;"><i class="fa-solid fa-plus"></i> &nbsp เพิ่มปัญหา Dowtime ระบบ</a>
                    </div>
            ...
            <div class="row" style="margin:5px;">
                <div class="input-group col-md-8 col-sm-8 col-lg-8">
                    <form action="index.php" class="search-container" method="POST" style="width: 100%;">
                        <input type="text" id="search-bar" placeholder="กรุณากรอกข้อมูลเพื่อค้นหา" name="empname" class="form-control">
                        <!-- <input type="submit" name="dowtimedata" value="ค้นหา" id="search-button" style="padding: 5px;width: 120px;"> -->
                        <button type="submit" name="dowtimedata" value="ค้นหา" id="search-button" style="padding: 5px;width: 120px;"><i class="fa-solid fa-magnifying-glass"></i> &nbsp ค้นหา</button>
                    </form>
                </div>
    ...
    <table class="table table-bordered">
        <thead class="thead-dark">
            <tr>
                <th>ลำดับ</th>
                <th>วันและเวลาที่แก้ไข</th>
                <th>ปัญหาระบบ</th>
                <th>สาเหตุที่ทำให้เกิดปัญหา</th>
                <th>วิธีแก้ไขปัญหา</th>
                <th>วันและเวลาที่แก้ไขเสร็จ</th>
            ...
            ?>
                <tr>
                    ...
                    <td>
                        ...
                        echo date_format($date, "d/M/Y "); echo "<br>".$row["timee"];
                        ?>
                    ...
                    <td><?php
                        $data_date = $row["editdate"] . ' ' . $row["edittime"];
                        $date = date_create($data_date);
                        echo date_format($date, "d/M/Y "); echo "<br>".$row["edittime"];
                        ?>

                    <td><?php
                        $data_date = $row["datee"] . ' ' . $row["timee"];
                        $data_date2 = $row["editdate"] . '' . $row["edittime"];
                    ...
                    <td>
                        ...
                        <?php if (!$row["signature_data"]) : ?>
                            <a href="signature.php?id=<?= $row["id"] ?>" role="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#operatorModal<?= $operator ?>">
                    ...
                    <td>
                        <a href="delete.php?idemp=<?php echo $row["id"]; ?>" class="btn btn-danger" onclick="return confirm('คุญต้องการลบหรือไหม ?')">ลบข้อมูล</a>
                    </td>
